...
#
# Usage:
#   ./deploy.sh              # Interactive mode (asks questions)
#   ./deploy.sh --dev        # Skip prompts, use development defaults
#   ./deploy.sh --prod       # Skip mode prompt, ask for domain/TLS details
#
set -euo pipefail
...
if [[ "$MODE" == "development" ]]; then
    ...
    echo ""
    echo "  Next steps:"
    echo "    1. Open the dashboard and log in with ADMIN_SECRET from .env"
    echo "    2. Generate org certs and onboard agents:"
