...
| `not a directory: mount src=... dst=...` on `compose up` | Orphan container shim from a previous failed `compose up` (P3 MINOR-I). The crashed container left its bind-mount dst auto-created as a directory; subsequent retries see the file/dir type mismatch. `./deploy.sh` now sweeps these shims before every `up`, but if you hit the error from a parallel `docker compose` invocation: `docker rm -f $(docker ps -aq --filter "label=com.docker.compose.project=cullis-mastio") && ./deploy.sh`. |
