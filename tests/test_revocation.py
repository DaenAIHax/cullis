...
async def test_revoked_cert_blocked(client: AsyncClient, dpop):
    ...
    proof2 = dpop.proof("POST", "/auth/token")
    resp = await client.post(
        "/auth/token",
...
async def test_double_revocation_returns_409(client: AsyncClient):
    ...
    resp = await client.post("/admin/certs/revoke", json=payload, headers=headers)
    assert resp.status_code == 200
...
async def test_list_revoked_certs(client: AsyncClient):
    ...
    resp = await client.get("/admin/certs/revoked",
                            headers={"x-admin-secret": ADMIN_SECRET})
    assert resp.status_code == 200
...
async def test_list_revoked_certs_filtered_by_org(client: AsyncClient):
    ...
    resp = await client.get(f"/admin/certs/revoked?org_id={org_id}",
                            headers={"x-admin-secret": ADMIN_SECRET})
    assert resp.status_code == 200
    records = resp.json()
    ...
    resp2 = await client.get("/admin/certs/revoked?org_id=org-nonexistent",
                             headers={"x-admin-secret": ADMIN_SECRET})
