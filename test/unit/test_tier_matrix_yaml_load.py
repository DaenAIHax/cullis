...
@pytest.mark.skipif(
    not DEFAULT_MATRIX_PATH.exists(),
    reason="bundled capability-tiers.yaml lives in enterprise-kit, not in "
           "the public core repo; the default loader falls back to a "
           "permissive matrix here (see _FALLBACK_DEFAULT_TIER).",
)
def test_load_bundled_default_yaml_parses_cleanly():
    """The repo-bundled default YAML must always parse — it ships with
    every Mastio container (when present)."""
