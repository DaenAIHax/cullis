...
def test_unparseable_versions_sort_first():
    """Garbage tag (``"latest"``, ``"main"``, manual experiments) must
    not silently win the comparison and trigger an "update available"
    suggestion. The core intentionally sorts garbage BEFORE anything
    parseable (version_check.py: returns ``(-1, v)``) so it never wins
    ``max()`` — previously garbage sorted AFTER and a 4-digit hotfix tag
    could win against a real release. Contract updated accordingly."""
    assert _version_key("garbage-tag") < _version_key("0.3.0-rc1")
    # …and ``unknown`` is treated like garbage too.
    assert _version_key("unknown") < _version_key("0.3.0")
