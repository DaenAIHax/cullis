...
class _LogCapture:
    """``caplog`` stand-in that captures ``watcher_mod.logger`` records.

    The ``mcp_proxy.lifespan.cert_expiry_watcher`` logger has
    ``propagate=False`` in the assembled app, so pytest's root-attached
    ``caplog`` never sees its records when the file runs inside the full
    suite (green in isolation, blank in suite). Capture the formatted
    messages by monkeypatching ``logger.warning`` / ``logger.info``
    directly — ``.text`` mirrors ``caplog.text`` so the substring
    assertions below stay byte-for-byte identical (same invariant).
    """
    ...
    def _record(self, msg, *args, **kwargs) -> None:
        self._messages.append(str(msg) % args if args else str(msg))

    def set_level(self, *args, **kwargs) -> None:  # caplog API parity
        return None
...
def captured_logs(monkeypatch):
    """Capture ``watcher_mod.logger`` warning/info output via monkeypatch.

    See ``_LogCapture`` — replaces ``caplog`` for the propagate=False
    watcher logger so the assertions survive the full-suite run.
    """
    cap = _LogCapture()
    monkeypatch.setattr(watcher_mod.logger, "warning", cap._record)
    monkeypatch.setattr(watcher_mod.logger, "info", cap._record)
    return cap
...
@pytest.mark.asyncio
async def test_org_root_above_threshold_is_silent(captured_audit, captured_logs):
    _, cert = _mint_cert("test-org-root", days_until_expiry=3000)
    mgr = _StubAgentManager(org_ca_cert=cert)
    now = datetime.datetime.now(datetime.timezone.utc)

    captured_logs.set_level("WARNING", logger="mcp_proxy.lifespan.cert_expiry_watcher")
    await watcher_mod._check_org_root(mgr, now, threshold_days=1825)

    assert captured_audit == []
    assert "Org Root" not in captured_logs.text


@pytest.mark.asyncio
async def test_org_root_below_threshold_warns_and_audits(captured_audit, captured_logs):
    ...
    assert "Org Root CA expires in" in captured_logs.text
...
async def test_intermediate_fallback_below_threshold_emits_info(
    captured_audit, captured_logs,
):
    _, cert = _mint_cert("test-intermediate", days_until_expiry=400)
    mgr = _StubAgentManager(mastio_ca_cert=cert)
    now = datetime.datetime.now(datetime.timezone.utc)

    captured_logs.set_level("INFO", logger="mcp_proxy.lifespan.cert_expiry_watcher")
    ...
    assert "Intermediate CA expires in" in captured_logs.text
...
@pytest.mark.asyncio
async def test_mastio_leaf_below_threshold_warns(captured_audit, captured_logs):
...
@pytest.mark.asyncio
async def test_mastio_leaf_malformed_pem_is_skipped(captured_audit, captured_logs):
    ...
    assert captured_audit == []
    assert "mastio leaf cert parse failed" in captured_logs.text
...
async def test_agent_leaves_per_agent_audit_uses_agent_id(
    captured_audit, captured_list_agents, captured_logs,
):
...
async def test_agent_leaves_malformed_cert_is_logged_but_does_not_break_loop(
    ...
    assert "agent-broken cert parse failed" in captured_logs.text
    matching = [r for r in captured_audit if r["action"] == "pki.agent_cert_expiry_warning"]
    assert len(matching) == 1
...
@pytest.mark.asyncio
async def test_nginx_server_below_threshold_warns(captured_audit, tmp_path, captured_logs):
