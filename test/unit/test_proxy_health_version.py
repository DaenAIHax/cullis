...
async def test_mastio_health_surfaces_module_version(monkeypatch):
    ...
    import mcp_proxy.main as _m
    # The container reads MCP_PROXY_VERSION; the bundle compose maps
    # CULLIS_MASTIO_VERSION (operator-facing) -> MCP_PROXY_VERSION
    # (container-facing). The test runs in-process with no compose layer,
    # so it sets the name the code actually reads.
    monkeypatch.setenv("MCP_PROXY_VERSION", "1.2.3")
