...
def _current_head() -> str:
    # Read the alembic head dynamically so a new migration doesn't break this
    # test (it pinned 0043 against the old .core-ref; HEAD moves with each PR).
    import mcp_proxy
    from alembic.script import ScriptDirectory
    _loc = Path(mcp_proxy.__file__).parent / "alembic"
    _cfg = AlembicConfig()
    _cfg.set_main_option("script_location", str(_loc))
    return ScriptDirectory.from_config(_cfg).get_current_head()


HEAD_REVISION = _current_head()
...
def _alembic_cfg(url: str) -> AlembicConfig:
    ...
    """
    import mcp_proxy
    _base = Path(mcp_proxy.__file__).parent
