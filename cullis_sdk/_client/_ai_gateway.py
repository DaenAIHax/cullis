...
class _AiGatewayMixin:
    ...
    def chat_completion(
        ...
        Wire path: Mastio's chat router is gated by
        ``get_agent_from_dpop_client_cert``, which verifies the DPoP
        proof's JWK thumbprint against the persistent ``internal_agents
        .dpop_jkt`` pinned at enrollment time. We therefore go through
        the **egress** surface (cert + persistent egress DPoP key from
        ``identity/dpop.jwk``), not through the bearer-DPoP wrapper
        ``_authed_request`` whose key is the ephemeral one generated by
        ``login_via_proxy*``. The egress wrapper also handles the
        ``use_dpop_nonce`` retry on cold start.
    ...
    def chat_completion_stream(
        ...
        Same egress path as :meth:`chat_completion` (persistent egress
        DPoP key, matches the cert-pinned route on Mastio). Cannot
        reuse :meth:`_egress_http` because it consumes the whole
        response into a buffer; we replicate its ``use_dpop_nonce``
        retry loop inline against ``self._http.stream`` so a 401 with
        ...
        try:
            if resp.status_code == 401:
                # Read enough of the body to spot the use_dpop_nonce
                # marker, then close + retry with the rotated nonce.
                body_bytes = b""
                for chunk in resp.iter_bytes():
                    body_bytes += chunk
                    if len(body_bytes) > 4096:
                        break
                marker_seen = b"use_dpop_nonce" in body_bytes
                fresh_nonce = (resp.headers or {}).get("dpop-nonce")
                cm.__exit__(None, None, None)
                cm = None
                if marker_seen and fresh_nonce:
                    self._egress_dpop_nonce = fresh_nonce
                    cm = _open()
                    resp = cm.__enter__()
                else:
                    # Genuine 401 (jkt mismatch / cert reject) — surface
                    # an HTTPStatusError carrying the upstream body so
                    # callers can read the ``detail``. Build a synthetic
                    # ``httpx.Response`` rather than reusing ``resp``
                    # whose stream is already closed.
            ...
            resp.raise_for_status()
            # Cache any nonce the server rotated on success so the next
            # call does not pay the retry round trip.
            rotated = (resp.headers or {}).get("dpop-nonce")
            if rotated:
