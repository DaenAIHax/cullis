...
async def log_audit(
    ...
    *,
    ...
    # ADR-032 Layer 2 — same pattern. Auth deps that successfully verify
    # a Connector ``X-Cullis-Session-Token`` stamp the user principal_id
    # in :mod:`mcp_proxy.auth.user_context`; fall back to it here so the
    # column populates without threading the value through call sites.
    if on_behalf_of_user_id is None:
        from mcp_proxy.auth.user_context import current_on_behalf_of_user
        on_behalf_of_user_id = current_on_behalf_of_user()
...
# ─────────────────────────────────────────────────────────────────────────────
# user_sessions (ADR-032 Layer 2 — Connector OIDC login)
# ─────────────────────────────────────────────────────────────────────────────
...
async def create_user_session(
    *,
    ...
    """Insert a fresh row into ``user_sessions``.

    ``session_id`` is the opaque token the Connector keeps and re-sends
    in ``X-Cullis-Session-Token``. It must be a cryptographically random
    string generated by the caller (``secrets.token_urlsafe`` is enough).
    Storing it verbatim is acceptable here because the row is the source
    of truth — a leak of the table is already game-over.

    ADR-033 Phase 2: ``user_signed_assertion`` (canonical JSON string of
    the WebAuthn AssertionResponse) and ``user_credential_id`` (the
    matching public-key credential id) are populated when the caller
    has verified a fresh assertion. ``maybe_stamp_user_session`` checks
    these columns at read time: NULL on a session row drives the warn /
        ...
        await conn.execute(
            text(
                """INSERT INTO user_sessions (
                       session_id, principal_id, agent_cert_thumbprint,
                       sso_subject, idp_issuer, display_name,
                       created_at, expires_at, revoked_at,
                       user_signed_assertion, user_credential_id
                   ) VALUES (
                       :session_id, :principal_id, :agent_cert_thumbprint,
                       :sso_subject, :idp_issuer, :display_name,
            ...
            ),
            {
                ...
                "principal_id": principal_id,
                "agent_cert_thumbprint": agent_cert_thumbprint,
...
async def get_user_session(session_id: str) -> dict | None:
    """Return the session row keyed by ``session_id``, or None when absent."""
    async with get_db() as conn:
        row = (await conn.execute(
...
async def revoke_user_session(session_id: str) -> bool:
    """Mark a session as revoked. Returns True iff a row was updated."""
    ts = datetime.now(timezone.utc).isoformat()
    async with get_db() as conn:
        result = await conn.execute(
            text(
...
async def upsert_local_user_principal_sso(
    *,
    ...
    """Insert or update the SSO identity metadata on ``local_user_principals``.

    Mirrors the pattern in ``mcp_proxy.admin.users.upsert_from_csr`` but
    targets the SSO columns added in migration 0034. Stays write-only;
    the read side goes through the dashboard's existing user lister.
    """
    ts = datetime.now(timezone.utc).isoformat()
    async with get_db() as conn:
        existing = (await conn.execute(
